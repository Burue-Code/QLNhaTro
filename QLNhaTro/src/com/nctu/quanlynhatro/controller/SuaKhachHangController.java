...
public class SuaKhachHangController {
    ...
    public SuaKhachHangController(ThemKhachHangView view, KhachHangController parentController,KhachHang khachHangCanSua ) {
        this.view = view;
    ...
    private void initEvents() {
        ...
        // Nút Cập nhật (View bạn đặt tên nút là "Thêm Mới" nhưng dùng chung form thì vẫn lấy getBtnThem)
    ...
    private void fillData() {
        if (khachHangCanSua == null) return;
    ...
    private void xuLyCapNhat() {
        try {
            ...
            // Chuyển đổi ngày sinh từ String -> LocalDate (Nếu bạn dùng JDateChooser thì getDate() rồi convert sẽ chuẩn hơn)
            if (!ngaySinh.isEmpty()) {
                try {
                    khachHangCanSua.setNgaySinh(java.time.LocalDate.parse(ngaySinh));
                } catch (Exception ex) {
                    JOptionPane.showMessageDialog(view, "Ngày sinh không đúng định dạng (YYYY-MM-DD)!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                    return;
                }
            } else {
            ...
            // 6. Xử lý kết quả
            if (kq) {
                JOptionPane.showMessageDialog(view, "Cập nhật khách hàng thành công!", "Thành công", JOptionPane.INFORMATION_MESSAGE);
                if (parentController != null) {
                    parentController.refreshData(); // Gọi lại hàm initData() bên KhachHangController để làm mới bảng
                }
                view.dispose(); // Đóng form
            } else {
                JOptionPane.showMessageDialog(view, "Lỗi khi cập nhật vào cơ sở dữ liệu!", "Lỗi", JOptionPane.ERROR_MESSAGE);
            }
