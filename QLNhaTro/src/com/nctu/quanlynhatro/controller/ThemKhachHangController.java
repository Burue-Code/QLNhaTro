...
public class ThemKhachHangController {
    ...
    private void xuLyThemMoi() {
        ...
        String ngaySinhStr = view.getNgaySinh();
        
        // Giả sử: false = Nam, true = Nữ
        boolean gioiTinh = view.getGioiTinh().equals("Nữ"); 

        // 2. Validate dữ liệu cơ bản
        if (tenKH.isEmpty()) {
            JOptionPane.showMessageDialog(view, "Vui lòng nhập tên khách hàng!", "Cảnh báo", JOptionPane.WARNING_MESSAGE);
            return;
        }
        if (!cccd.matches("\\d{12}")) {
            JOptionPane.showMessageDialog(view, "CCCD/CMND phải bao gồm đúng 12 chữ số!", "Lỗi nhập liệu", JOptionPane.ERROR_MESSAGE);
            return;
        }
        if (!sdt.matches("0\\d{9}")) {
            JOptionPane.showMessageDialog(view, "Số điện thoại không hợp lệ (10 số, bắt đầu bằng 0)!", "Lỗi nhập liệu", JOptionPane.ERROR_MESSAGE);
            return;
        }

        // 3. Xử lý ép kiểu dữ liệu
        java.time.LocalDate ngaySinh = null;
        if (!ngaySinhStr.isEmpty()) {
            try {
                // Chuyển chuỗi thành LocalDate (Định dạng bắt buộc: YYYY-MM-DD)
                ngaySinh = java.time.LocalDate.parse(ngaySinhStr);
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(view, "Ngày sinh không đúng định dạng (Năm-Tháng-Ngày, VD: 2000-12-30)!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                return;
            }
        }

        long maKHC = 0; // Mặc định là 0 (Không ghép phòng)
        ...
        try {
            // 4. Gọi DAO để INSERT xuống CSDL
            // Truyền các tham số khớp với hàm insert trong KhachHangDAO của bạn:
            // insert(String tenKH, String diaChi, LocalDate ngaySinh, String sdt, boolean gioiTinh, String soCCCD, String gmail, long maKHC)
            ...
            if (kq) {
                ...
                // Gọi làm mới bảng bên giao diện chính
                if (parentController != null) {
                    parentController.refreshData();
                }
                view.dispose(); // Đóng form Thêm
            } else {
                JOptionPane.showMessageDialog(view, "Thêm thất bại. Vui lòng kiểm tra lại dữ liệu!", "Lỗi", JOptionPane.ERROR_MESSAGE);
            }
        } catch (Exception ex) {
            ex.printStackTrace(); // In lỗi ra console để dev dễ nhìn
    ...
    private void initSearch() {
        ...
        view.getTxtTimKiem().addKeyListener(new KeyAdapter() {
            ...
            public void keyReleased(KeyEvent e) {
                ...
                sorter.setRowFilter(
                        text.isBlank() ? null : RowFilter.regexFilter("(?i)" + text)
                );
            }
        });
    }
